...
//    @SystemLog1(operation = "新增用户信息测试操作。。。。。")
    @PostMapping("addUser")
    public AppResponse addUser(@Valid UserInfo userInfo) {
        try {
            //获取用户id
...
//    @SystemLog1(operation = "获取用户列表。。。。。")
    ...
    public AppResponse updateUserById(@Valid UserInfo userInfo) {
        try {
            //获取用户id
            String userId = SecurityUtils.getCurrentUserId();
            userInfo.setCreateBy(userId);
    ...
    public AppResponse findUserById(@NotNull(message = "用户代码不能为空") String userId) {
